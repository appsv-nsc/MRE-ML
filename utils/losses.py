...
class TargetMaskWeightedMSELoss(nn.Module):
    def __init__(self, mul_factor=10):
        super(TargetMaskWeightedMSELoss, self).__init__()
        self.multiply_factor = mul_factor

    def forward(self, pred, target, seg_mask):
        error = pred - target
        weights = (seg_mask.unsqueeze(1) * self.multiply_factor) + target + 1
        weighted_error = (error ** 2) * weights
